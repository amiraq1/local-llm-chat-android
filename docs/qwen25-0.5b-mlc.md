# Qwen 2.5 0.5B Instruct MLC Android

`Qwen2.5-0.5B-Instruct-q4f16_1-MLC` is registered in the app catalog as a
...
```text
scripts/mlc-package-qwen25-0.5b.json
```
...
`Qwen2.5-0.5B-Instruct-q4f16_1-MLC` into:
...
The current bundled runtime does not contain a Qwen2.5 model library, so adding
only the catalog entry is not enough to run the model.
